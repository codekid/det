...
| Relation `load` | Closed set: `full_refresh` → silver table (stg view); `parent_replace` → silver incremental delete+insert on delete key `[parent_key]+ancestor spine`, stg view, `tags=["det_catchup"]`. On BigQuery, scaffold emits `delete+insert` (not `merge`) so vanished children are removed; DET `det_bq_delete_insert` macros enable that strategy. Empty/null child arrays are cleared via `pre_hook=det_relation_clear_empty_arrays()` (no key-only inserts). Conflicts with `materialized: table` + `parent_replace`. Dedupe still uses full spine. Not more adapt knobs — finishes nested load semantics. |
