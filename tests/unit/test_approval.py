...
def test_bound_params_match_write_argv_signatures():
    ...
    builders = {
        ...
        "load": load_write_argv,
        "run": run_write_argv,
        "migrate": migrate_write_argv,
        "prune": prune_write_argv,
        "dbt": dbt_write_argv,
        "scaffold-dbt": scaffold_dbt_write_argv,
        "scaffold-ops": scaffold_ops_write_argv,
        "init-pipeline": init_pipeline_write_argv,
        "biglake-register": biglake_register_write_argv,
        "iceberg-register": iceberg_register_write_argv,
        "lock-release": lock_release_write_argv,
        "silver-catchup-plan": silver_catchup_plan_write_argv,
        "silver-catchup-cleanup": silver_catchup_cleanup_write_argv,
    }
    assert set(builders) == set(APPROVAL_BOUND_PARAMS)
    for command, fn in builders.items():
        bound = APPROVAL_BOUND_PARAMS[command]
        cli_only = CLI_ONLY_BOUND_PARAMS.get(command, frozenset())
        params = set(inspect.signature(fn).parameters)
        assert params <= bound, f"{command}: builder kwargs not in bound: {params - bound}"
        assert (bound - cli_only) <= params, (
            f"{command}: bound params missing from builder (add to signature or "
            f"CLI_ONLY_BOUND_PARAMS): {(bound - cli_only) - params}"
        )
...
def test_bound_params_spot_check_argv_tokens():
    ...
    assert "--keep" in prune_keep and prune_keep != prune_base
    dbt_base = dbt_write_argv(None, command="build")
    dbt_fr = dbt_write_argv(None, command="build", full_refresh=True)
