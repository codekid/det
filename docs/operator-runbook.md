...
| Lease held / fence | `det lock-show -p … -s …` · receipts under `{ops}/runs/` | `diagnose_pipeline` · `list_runs` | Wait for TTL / steal, or `det lock-release … --force` after the worker is gone. Prefer one writer per pipeline+interval so the residual fence→publish window stays moot (accepted v1; [publication-contract.md](publication-contract.md)) | `--force` release while a writer may still be alive |
...
| Silver hole (latest bronze missing in silver) | Mode A heal checklist in [silver-catchup.md](silver-catchup.md) (`heal` → approve/apply → `heal --continue` → approve/build → verify) | `diff_bronze_silver` · `silver_catchup_heal_dry_run` (apply rung) then later heal continue / `dbt_dry_run(catchup=…)` (build rung) | Follow Mode A happy path; separate approve per rung | Full-refresh silver as the first heal; chain dry-run → write; apply+build same turn; start with `--census` |
