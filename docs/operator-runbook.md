...
| --- | --- | --- | --- | --- |
| Approval stuck `claimed` (crash between claim and consume) | `det list-approvals --status claimed` · `det approval-show <id>` (heartbeat_status) | `list_approvals` `status="claimed"` · `describe_approval` | Prefer new `det approve`. Or `det approval-release <id> --force` only after the worker is confirmed dead (warns if heartbeat fresh). Store: `{ops}/approvals/` or Postgres | Release while a run may still be alive; auto-TTL release; describe approvals as auth |
