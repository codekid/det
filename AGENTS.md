...
| Bronze↔silver catch-up | **One rung per turn.** Mode A boring route (default `48h`): MCP `diff_bronze_silver` → `silver_catchup_heal_dry_run` → show `approval_plan` / `next_rung=apply`, **stop** → after confirm: `det approve` → later-turn apply (`det silver-catchup apply -p <pipeline> --extract-lookback 48h --manifest-id <scm_…> --content-digest <sha256:…> --approval <id>` — same scope as `approval_plan`). **After apply succeeds:** CLI `det silver-catchup heal --continue --manifest-id <scm_…>` or MCP `dbt_dry_run(catchup=True, catchup_manifest=<scm_…>)` → show `next_rung=build`, **stop** → separate approve → later-turn build. Then verify. Advanced: `silver_catchup_dry_run` / `census=true`. BQ cleanup after verify is a separate cleanup rung. Never chain dry-run→write or apply+build in the same turn. Details: [docs/silver-catchup.md](docs/silver-catchup.md). |
